...
def MultiPie(mydf: pd, mytitle) -> plt:
    ...
    # fig.legend(handles, labels,  bbox_to_anchor=(0.2, 0.8))
    fig.legend(labels, loc='upper left', ncol=1)
...
def QRadar(question) -> None:
    ...
    for r in range(1, int(max_val) + 1):
...
def Plot_bar(question, maxpage):
    ...
    plt.ylim((0, 5))
    plt.ylabel("Rating")
    title = f"{question}: {occurences[question].get('title')}"
    plt.title(title)
    # plt.show()
    plt.savefig(f"{remplacer_caracteres(title)}_hist.svg")
    plt.close(fig)
...
def generate_charts(change) -> str:
    ...
    try:
        ...
        raise UserWarning(f"Problème de lecture du fichier. Vérifier le nom du fichier .CSV\nSaisir le bon nom de fichier à l'étape précédente.\n{e}")
    ...
    for col in _extra_cols:
        try:
            ...
            df.drop(df.columns[position], axis=1, inplace=True)
        except KeyError:
    ...
    names = np.sort(df[course_col].dropna().unique())
    ...
    for q in occurences:
        if occurences[q]['type'] == 'pie':
            ...
            if df[occurences[q]['column'][0]].isna().all().all() != True:
                ...
                pages.append(('img', f"{q}: {occurences[q]['title']}", occurences[q].get('imgs')))
            ...
            if len(paragraph.split('\n')) >=2:
                pages.append(('txt', f"{q}: {occurences[q]['title']}", paragraph))
